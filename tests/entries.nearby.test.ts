...
describe('GET /entries/nearby', () => {
  it('returns only entries within radiusKm, sorted nearest first, with distanceKm attached', async () => {
    const near = await authedRequest(app).post('/entries').send({
      placeName: 'Near Cafe',
      neighborhood: 'Origin',
      city: 'Testville',
      lat: 0.009,
    ...
    const far = await authedRequest(app).post('/entries').send({
      placeName: 'Far Cafe',
      neighborhood: 'Origin',
      city: 'Testville',
      lat: 0.45,
    ...
    const small = await authedRequest(app).get('/entries/nearby').query({ lat: 0, lng: 0, radiusKm: 5 });
    ...
    const big = await authedRequest(app).get('/entries/nearby').query({ lat: 0, lng: 0, radiusKm: 100 });
  ...
  it('defaults radiusKm to 5 when omitted', async () => {
    ...
    await authedRequest(app).post('/entries').send({
      placeName: 'Far Cafe',
      neighborhood: 'Origin',
      city: 'Testville',
      lat: 0.45,
    ...
    const res = await authedRequest(app).get('/entries/nearby').query({ lat: 0, lng: 0 });

    expect(res.body.map((e: { id: string }) => e.id)).toEqual([near.body.id]);
  });

  it('rejects a request missing lat/lng', async () => {
    const res = await authedRequest(app).get('/entries/nearby').query({ radiusKm: 5 });
    expect(res.status).toBe(400);
  });

  it('does not treat "nearby" as an :id lookup', async () => {
    const res = await authedRequest(app).get('/entries/nearby').query({ lat: 0, lng: 0, radiusKm: 5 });
